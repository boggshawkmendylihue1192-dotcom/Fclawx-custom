...
export async function handleProviderRoutes(
  ...
  if (url.pathname.startsWith('/api/provider-accounts/') && url.pathname.endsWith('/sync-runtime') && req.method === 'POST') {
    const accountId = decodeURIComponent(
      url.pathname.slice('/api/provider-accounts/'.length, -'/sync-runtime'.length),
    );
    try {
      ...
      const apiKey = await providerService.getAccountApiKey(accountId);
      await syncUpdatedProviderToRuntime(providerAccountToConfig(account), apiKey ?? undefined, ctx.gatewayManager);
      sendJson(res, 200, {
        success: true,
        hasKey: Boolean(apiKey),
        keyTail: apiKey ? apiKey.slice(-8) : null,
      });
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
    }
