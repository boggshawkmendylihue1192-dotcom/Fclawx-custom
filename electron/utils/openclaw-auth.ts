...
export async function saveProviderKeyToOpenClaw(
  ...
  for (const id of agentIds) {
    ...
    const launcherProfileId = `${provider}:launcher`;

    store.profiles[defaultProfileId] = { type: 'api_key', provider, key: apiKey };
    store.profiles[launcherProfileId] = { type: 'api_key', provider, key: apiKey };
    ...
    store.order[provider] = [
      defaultProfileId,
      launcherProfileId,
      ...existingOrder.filter((profileId) => (
        profileId !== defaultProfileId
        && profileId !== launcherProfileId
      )),
    ];

    if (!store.lastGood) store.lastGood = {};
    store.lastGood[provider] = defaultProfileId;
...
export async function removeProviderKeyFromOpenClaw(
  ...
  for (const id of agentIds) {
    const store = await readAuthProfiles(id);
    const removedDefault = removeProfileFromStore(store, `${provider}:default`, 'api_key');
    const removedLauncher = removeProfileFromStore(store, `${provider}:launcher`, 'api_key');
    if (removedDefault || removedLauncher) {
      await writeAuthProfiles(store, id);
    }
  }
