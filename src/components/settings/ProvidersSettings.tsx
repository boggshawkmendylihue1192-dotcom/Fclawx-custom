...
export function ProvidersSettings() {
  ...
  const handleSyncProvider = async (providerId: string) => {
    try {
      const result = await syncAccountRuntime(providerId);
      toast.success(
        result.keyTail
          ? `已同步到 OpenClaw 运行配置，当前密钥尾号 ${result.keyTail}`
          : '已同步到 OpenClaw 运行配置'
      );
    } catch (error) {
      toast.error(`同步到 OpenClaw 失败: ${error}`);
    }
  };
        ...
        <div className="space-y-3">
          {displayProviders.map((item) => (
            <ProviderCard
              ...
              onSyncRuntime={() => handleSyncProvider(item.account.id)}
      ...
      <div className="flex items-center justify-between">
        ...
        {!isEditing && (
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <Button
              data-testid={`provider-sync-${account.id}`}
              variant="ghost"
              size="icon"
              className="h-8 w-8 rounded-full text-muted-foreground hover:text-blue-600 hover:bg-white dark:hover:bg-card shadow-sm"
              onClick={handleSyncRuntime}
              disabled={syncing}
              title="同步到 OpenClaw"
            >
