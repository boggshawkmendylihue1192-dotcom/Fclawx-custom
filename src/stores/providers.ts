...
interface ProviderState {
  ...
  syncAccountRuntime: (accountId: string) => Promise<{ hasKey: boolean; keyTail: string | null }>;
...
export const useProviderStore = create<ProviderState>((set, get) => ({
  ...
  syncAccountRuntime: async (accountId) => {
    const result = await hostApiFetch<{
      success: boolean;
      error?: string;
      hasKey?: boolean;
      keyTail?: string | null;
    }>(`/api/provider-accounts/${encodeURIComponent(accountId)}/sync-runtime`, {
      method: 'POST',
    });

    if (!result.success) {
      throw new Error(result.error || 'Failed to sync provider account to OpenClaw runtime');
    }

    await get().refreshProviderSnapshot();
    return {
      hasKey: Boolean(result.hasKey),
      keyTail: result.keyTail ?? null,
    };
  },
